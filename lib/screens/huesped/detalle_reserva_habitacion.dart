...
class DetalleReservaHuespedScreen extends StatefulWidget {
  final dynamic habitacion;
  const DetalleReservaHuespedScreen({Key? key, this.habitacion}) : super(key: key);

  @override
  State<DetalleReservaHuespedScreen> createState() => _DetalleReservaHuespedScreenState();
}

class _DetalleReservaHuespedScreenState extends State<DetalleReservaHuespedScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    String rawServicios = widget.habitacion['servicios'] ?? ''; // "{Aire Acondicionado,Jacuzzi}"
    
List<String> servicios = rawServicios
    .replaceAll(RegExp(r'[\{\}]'), '') // elimina las llaves
    .split(',')                        // divide por coma
    .map((e) => e.trim().replaceAll('"', '')) // limpia espacios y comillas
    .toList();
